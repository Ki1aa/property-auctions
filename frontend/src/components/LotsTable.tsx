        ...
        {lots.map((lot) => (
          <tr key={lot.id}>
            <td>
              ...
              {lot.is_izhs_candidate && <span className="badge badge--success" style={{ marginLeft: 8 }}>ИЖС</span>}
            </td>
            <td><StatusBadge status={lot.status} /></td>
            <td>{lot.region || "—"}</td>
            <td>{formatArea(lot.area_sqm)}</td>
            <td className="cell--mono">{lot.cadastral_number || "—"}</td>
            <td>{formatPrice(lot.current_price ?? lot.start_price)}</td>
            <td>{formatDate(lot.end_date)}</td>
          </tr>
        ))}
      </tbody>
    </table>
