...
backend/
  app/
    ...
    models.py          # SQLAlchemy: Organizer, Lot (+municipality/settlement/VRI codes, map_anchor_*), LotNoticeAttribute (EAV characteristics из detail JSON), LotSnapshot, IngestRun, IngestManifest, AlertEvent, OpenDataNotice, MarketComparable
  ...
  alembic/
    env.py
    versions/          # Alembic-миграции: init, opendata_notices, ingest_manifest, lot land/municipality/notice identity, NSPD, market_comparables
  scripts/
...
Закрытое окно без сегодняшнего дня (по умолчанию последние N дней до вчера):
...
**Технические правила:**
- В dev-режиме на **SQLite** при старте вызывается `Base.metadata.create_all()` в [backend/app/main.py](backend/app/main.py); для **PostgreSQL** автосоздание отключено — схема через Alembic. При изменении моделей в [backend/app/models.py](backend/app/models.py) **обязательно** создавать Alembic-ревизию **И** запускать [backend/scripts/dev_sync_schema.py](backend/scripts/dev_sync_schema.py) для существующей SQLite, потому что `create_all()` не делает ALTER на уже существующих таблицах. `dev_sync_schema.py` добавляет недостающие колонки и индексы; внешние ключи в существующей SQLite-таблице не добавляет, а предупреждает о необходимости fresh DB/rebuild/Alembic.
...
- **Связь Lot ↔ OpenDataNotice**: FK `Lot.opendata_notice_id`, ingest пишет обе таблицы атомарно, `/api/lots/{id}` возвращает `notice_payload` (raw opendata-извещение), `notice_attributes` (все `characteristics` из detail JSON), поля `map_anchor_*` (канон для карт после `refresh_lot_map_anchor`: НСПД-центроид или координаты извещения). Если одно извещение содержит несколько `notice.lots[]`, detail-fetch разворачивает их в отдельные `Lot`, сохраняет стабильные `notice_reg_num`, `notice_lot_number`, `notice_lot_count`, а `source_id` остаётся совместимым (`regNum`, `regNum:lot:<lotNumber>`); cancel/stop/resumption/annulment events обновляют все строки этого извещения - [backend/app/models.py](backend/app/models.py), [backend/app/api.py](backend/app/api.py), [backend/app/services/ingest/service.py](backend/app/services/ingest/service.py).
- **ПКК на карточке лота:** только `GET /api/lots/{id}` при `NSPD_ENABLED` и `NSPD_RESOLVE_PKK_LINK_ON_DETAIL` может один раз вызвать геопортал и вернуть `pkk_map_url`/`nspd_map_url` с `selectedCard`, если участок найден; иначе прежний fallback с `query` по кадастру. Список `/api/lots` live-запрос не делает — [backend/app/services/nspd/resolve_pkk_link.py](backend/app/services/nspd/resolve_pkk_link.py).
- **Заголовок лота vs извещение:** в UI/CSV/карте поле `title` для списка строится через `lot_preferred_list_title` (имя лота из snapshot, кадастр, `Лот N · regNum`, не название извещения). Ingest OpenData: в `Lot.title` не кладётся `noticeName` — см. [backend/app/services/ingest/normalizer.py](backend/app/services/ingest/normalizer.py).
- REST API: `/health`, `/api/lots` (ответ `{ items, total, limit, offset }`; фильтры; `region` поддерживает один код, несколько повторяющихся query-параметров или строку с кодами через запятую; `sort`; `notice_reg_num`, `notice_lot_number`, `notice_lot_count` для объяснения multi-lot; `start_price_per_sotka` / `start_price_per_sqm` из извещения только при `start_price > 0`; `baseline_price_per_sotka`, `discount_to_baseline`, `valuation_confidence`; `category` повторяющимся query для OR по видам торгов; сортировка `discount_to_baseline_desc`; фильтры качества `has_cadastral`, `has_price_per_sotka`, `has_positive_discount`), `GET /api/export/lots.csv` (те же фильтры + baseline-колонки), `/api/lots/facets`, `/api/lots/quality`, `/api/lots/{id}` (+ `notice_payload`, `opendata_notice_id`, baseline-поля, `app_lot_url`, `torgi_url` на конкретный лот `/new/public/lots/lot/{regNum}_{lotNumber}/(lotInfo:info)`, отдельный `torgi_notice_url` на извещение `/new/public/notices/view/{regNum}`, `torgi_json_url`, `nspd_map_url` с `selectedCard` при наличии `nspd_card_id/type` и центроида, `domclick_map_url` по bbox вокруг центроида при `INCLUDE_MARKETPLACE_MAP_URLS=true`; `pkk_map_url` — та же публичная кадастровая карта (ПКК) на `nspd.gov.ru` с параметром `query` по кадастровому номеру; текстовый поиск Домклик (`domclick_search_url`, `domclick_search_url_cadastral`) при `INCLUDE_MARKETPLACE_SEARCH_URLS=true`), `/api/lots-map`, `/api/ingest-runs` (история запусков + `processed_files`, `failed_files`, `last_error_source_url`, `error_kind`), `/api/ingest-status`, `POST /api/ingest-runs/start`, `/api/opendata-notices` (ответ `{ items, total, limit, offset }`; фильтры `document_type` / `bidd_type_code` списками и `reg_num`; `sort` по `publish_date`, `reg_num`, `document_type`, `bidd_type_code` с направлением asc/desc), `/api/opendata-notices/facets` - [backend/app/api.py](backend/app/api.py).
...
- Telegram-уведомления (дедупликация через AlertEvent; опционально **digest** через `TELEGRAM_DIGEST_ENABLED` и таблицу `telegram_digest_items`; компактная карточка с вердиктом, блоком «Почему интересно», сигналами, ценой, baseline/рынок, `regNum + lotNumber`; для MVP UI/БД могут показывать все регионы, а Telegram сужается отдельным `TELEGRAM_ALERT_REGION_CODES=72` и добавляет примечание о регионе 72; low-signal алерты без ИЖС/кадастра/цены за сотку/baseline по умолчанию пропускаются через `TELEGRAM_ALERT_SKIP_LOW_SIGNAL=true`; фильтры `TELEGRAM_ALERT_ONLY_IZHS`, `TELEGRAM_ALERT_REQUIRE_CADASTRAL`, `TELEGRAM_ALERT_REQUIRE_DISCOUNT_OR_PER_SOTKA`, `TELEGRAM_ALERT_MIN_DISCOUNT_TO_BASELINE`, `TELEGRAM_ALERT_REQUIRE_BASELINE_FOR_DISCOUNT`; изменения существующих лотов отправляются в Telegram только при изменении цены; ссылки на монитор, ГИС Торги, ПКК, НСПД-карту, Домклик (карта района и при включении флага — текстовый поиск), raw JSON; обрезка длины, retry при 429, отключение превью ссылок, `TELEGRAM_PROXY_URL` при проблемах маршрута к Bot API; smoke `python scripts/send_telegram_test.py`) - [backend/app/services/alerts/](backend/app/services/alerts); пошаговая настройка в [README.md](README.md) (раздел Telegram-алерты).
