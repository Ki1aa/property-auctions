...
**MVP GIS (параллельные таблицы `mvp_gis_*`):** ingest и read-only API не заменяют текущий `/api/lots`. Команды: `cd backend && python scripts/backup_mvp_db.py` (копия SQLite), `python scripts/torgi_discover.py` (диагностика OpenData: meta.json и выбранные файлы), `python scripts/debug_find_land_lots.py` (последние срезы meta, гистограммы biddTypeCode/ZK, примеры земельных лотов без БД), `python scripts/ingest_torgi.py --dry-run --limit 50`, при необходимости один срез: `python scripts/ingest_torgi.py --dry-run --source-url "https://.../data-...structure-....json" --limit 100` (обходит discovery и не использует синтез срезов из `INGEST_SOURCE_URL`). Поля и коды земли — черновик чеклиста в [docs/GIS_TORGI_FIELD_RESEARCH.md](docs/GIS_TORGI_FIELD_RESEARCH.md). Сводка: `GET http://localhost:8000/api/mvp/stats`.
