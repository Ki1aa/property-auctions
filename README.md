...
После ingest при событиях `new_lot` или **значимом** изменении существующего лота (цена, статус, даты торгов, площадь, кадастр, флаг ИЖС, категория земель, ВРИ) backend может отправить компактную карточку в Telegram: вердикт, причина попадания, `regNum + lotNumber`, кадастр, земля, цена за сотку, baseline и ссылки на монитор, конкретный лот ГИС Торги `/new/public/lots/lot/{regNum}_{lotNumber}/(lotInfo:info)`, отдельное извещение `/new/public/notices/view/{regNum}`, ПКК, НСПД-карту/deep link, карту Домклик вокруг участка при наличии координат и сырой JSON извещения. Расширенный текстовый поиск Домклик опционален через `INCLUDE_MARKETPLACE_SEARCH_URLS=true`; в MVP он выключен по умолчанию (капча/пустая выдача).
...
- Земля по всей РФ в БД, Тюмень в Telegram: пустой `TARGET_REGION_CODES`, `INGEST_ONLY_LAND_LOTS=true`, `TELEGRAM_ALERT_REGION_CODES=72`, при необходимости `TELEGRAM_ALERT_ONLY_IZHS=true` — см. комментарий «MVP operator checklist» в [.env.example](.env.example).
- Карта Домклик вокруг участка: `INCLUDE_MARKETPLACE_MAP_URLS=true` и региональный `DOMCLICK_ON_MAP_BASE_URL`, например `https://tyumen.domclick.ru/search/on-map` (параметр `aids` при необходимости скопировать с рабочего URL в браузере).
- Опционально `NSPD_ENABLED=true` на той же машине, если доступен `nspd.gov.ru`; политики слияния `NSPD_MERGE_*` — в `.env.example`. Ingest перед Telegram вызывает обогащение: в ссылке НСПД появляются `selectedCard` и координаты, если геопортал вернул карточку. Для локального split tunneling с self-signed TLS цепочкой можно временно ставить `NSPD_VERIFY_TLS=false`, после чего `domclick_map_url` начнёт появляться у лотов с найденным центроидом.
