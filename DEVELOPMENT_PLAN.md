...
- Backend: [backend/app/services/external_lot_links.py](backend/app/services/external_lot_links.py) — генерация URL; настройки `APP_PUBLIC_BASE_URL`, `INCLUDE_MARKETPLACE_QUICK_LINKS`, `INCLUDE_MARKETPLACE_SEARCH_URLS`, шаблоны `DOMCLICK_SEARCH_TEMPLATE`, `AVITO_LAND_SEARCH_TEMPLATE`, `CIAN_LAND_SEARCH_TEMPLATE` в [backend/app/config.py](backend/app/config.py); поля `app_lot_url`, `torgi_url` (конкретный лот), `torgi_notice_url` (извещение), `torgi_json_url` (сырой href при отличии от HTML-ссылок), `nspd_map_url` (deep link через `selectedCard`, если есть NSPD card id/type и центроид; иначе карта с query/координатами), legacy-null `pkk_map_url`, быстрые кадастровые ссылки Авито/Циан и расширенные пары ссылок Домклик/Авито/Циан (`*_cadastral`) в ответах `/api/lots` и `/api/lots/{id}`.
